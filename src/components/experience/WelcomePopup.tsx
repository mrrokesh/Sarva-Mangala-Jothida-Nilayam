...
export function WelcomePopup({ onOpenChange }: { onOpenChange?: (open: boolean) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="welcome-title">
      <button className="absolute inset-0 bg-ink/70 backdrop-blur-sm" aria-label={t("close")} onClick={close} />
      <div className="card-metal relative grid max-h-[90vh] w-full max-w-3xl overflow-auto rounded-3xl md:grid-cols-2">
        <button
          onClick={close}
        ...
        <div className="relative min-h-[240px] overflow-hidden bg-royal-night">
