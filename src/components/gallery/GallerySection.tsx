...
export function GallerySection() {
  ...
  return (
    <>
      <Section id="gallery" title={isTa ? "புகைப்படங்கள்" : "Gallery"}>
        ...
        <div className="columns-1 gap-3 min-[420px]:columns-2 md:columns-3">
          {items.map((g, idx) => (
      ...
      {current && i !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
          ...
          <div className="relative z-10 max-w-4xl">
            ...
            <p className="mt-3 text-center text-white">{isTa ? current.ta : current.en}</p>
            <p className="text-center text-sm text-white/75">
              {isTa ? current.eventTa : current.eventEn} {current.year ? `· ${current.year}` : ""}
            </p>
            <div className="mt-3 flex justify-center gap-4">
              <button className="text-cyan" onClick={() => setI((n) => (n! > 0 ? n! - 1 : items.length - 1))}>
                ‹
              </button>
              <button className="text-cyan" onClick={() => setI((n) => (n! + 1) % items.length)}>
                ›
              </button>
            </div>
          </div>
